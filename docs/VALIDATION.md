...
- `npm test`：公开发行边界、配置默认值、账号过滤、命令发现、记忆保留、空项目库/损坏项目库等 9 项通过。
- `node scripts/audit-public.js`：公开文件集检查通过。排除了 19 个个人模块源文件；不带旧 Git 历史、运行数据与个人规则文档。
- `node tests/e2e-community-cdp.js`：真实隔离 Electron 中验证空白首页、安装检测、账号入口、授权按钮 IPC、通用/开发群聊入口、research 请求拒绝、Codex 原生协议夹具会话，以及 hook server 监听。
- 改动的 JavaScript 使用 `node --check` 检查；首次安装脚本的真实执行同时验证 PowerShell 语法与错误传播。
