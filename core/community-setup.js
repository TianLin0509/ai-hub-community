...
function findCommand(command, env = process.env, platform = process.platform) {
  ...
  if (platform === 'win32') dirs.push(path.join(env.APPDATA || '', 'npm'), path.join(env.USERPROFILE || os.homedir(), '.local', 'bin'));
...
function inspectSetup({ root = path.resolve(__dirname, '..'), env = process.env, platform = process.platform, version = process.versions.node } = {}) {
  ...
  const requirements = { windows: platform === 'win32', node22: Number(version.split('.')[0]) >= 22, electron: fs.existsSync(electron) };
  return { schemaVersion: 1, edition: 'community', requirements, ready: Object.values(requirements).every(Boolean), providers,
